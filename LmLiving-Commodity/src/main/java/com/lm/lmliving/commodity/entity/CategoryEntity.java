...
public class CategoryEntity implements Serializable {
	...
	/**
	 * 0不显示，1显示
	 * 1.如果没有在application.yml配置逻辑删除和未删除的值，也
	...
	/**
	 * 增加一个属性childrenCategories
	 * 1,childrenCategories表示某个分类的子分类集合
	 * 2.childrenCategories没有对应表commodity_category字段
	 * 3.@TableField(exist=false)表示childrenCategories不对应表的字段
	 * 4.@JsonInclude(JsonInclude.Include.NON_EMPTY)
	 * 如果childrenCategories的值为空数组，就不需要返回,就是下一级没有childrenCategory了，就不需要返回
	 */
