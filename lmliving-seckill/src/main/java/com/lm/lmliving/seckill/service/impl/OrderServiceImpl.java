...
import com.lm.lmliving.seckill.mapper.OrderMapper;
...
import com.lm.lmliving.seckill.util.MD5Util;
import com.lm.lmliving.seckill.util.UUIDUtil;
...
public class OrderServiceImpl
    ...
    public Order seckill(User user, GoodsVo goodsVo) {
        ...
        //说明：只要在更新成功时,返回true,否则返回false, 即更新后，受影响的行数>=1为T
        ...
        order.setDeliveryAddrId(0L);//就设置一个初始值
        order.setGoodsName(goodsVo.getGoodsName());
        order.setGoodsCount(1);
        order.setGoodsPrice(seckillGoods.getSeckillPrice());
        order.setOrderChannel(1);//就设置一个初始值
        order.setStatus(0);//就设置一个初始值-未支付
